...
# 处方状态：分组与「是否命中」的唯一依据（详见 _rx 的 docstring）
STATE_ORDER = {"alert": 0, "watch": 1, "unknown": 2, "ok": 3, "error": 4}
STATE_LABELS = {
    "alert": "本期行动清单",
    "watch": "观察项（弱信号）",
    "unknown": "待补样本（暂不出结论）",
...
VALID_STATES = tuple(STATE_ORDER)
...
def _rx(rid, module, severity, state, signal, actions, falsify, basis_note=""):
    """state 为**必填位置参数**（不设默认值）：漏标的分支会在运行期直接 TypeError 报出来，不会被静默当成「异常」。

    state 口径（决定处方单怎么分组、也是「换账号是否各自不同」的判定依据）：
      alert   命中异常信号 → 进「本期行动清单」
      watch   弱信号/机会，够不上异常但值得观察
      ok      该维度已达标或不是当前瓶颈（负向处方，压成一行，不再占卡片）
      unknown 样本不足/测不出来 → 不硬给结论，等补样本
    """
    return {
        "id": rid,
        "module": module,
...
def rx04_title(s):
    ...
    if d is None:
        return _rx("RX-04", "M3", "低", "unknown", "标题风格两组样本不足（疑问式 %d / 陈述式 %d），无法对照" % (q["n"], st["n"]),
                   ["两类标题各再发 3 篇，凑齐可比样本"],
                   "样本齐后若差异 ≥20% → 按胜出风格统一")
    if d >= 0.2:
        return _rx("RX-04", "M3", "中", "alert",
...
def rx06_comment(s):
    ...
    cs = s["comment_share"]
    if cs < 0.06:
        return _rx("RX-06", "M7", "高", "alert",
                   "评论仅占互动总量 %.1f%%（低于 6%% 健康线）——读者「赞了就走」，没有形成讨论场。"
                   "外部实证：评论数与阅读量的相关性为全部互动指标中最强（> 赞 > 藏，单一创作者 100w 阅读 300+ 篇样本，"
                   "非平台官方口径——用你自己的数据复核）" % (cs * 100),
                   ["每篇结尾加一个**可争议的具体问题**（不是「你怎么看」）",
                    "发布后 1 小时内自己置顶一条补充观点，给评论区定调",
                    "回复前 5 条评论（早期互动率会被算法二次放大）"],
                   "若评论占比升到 8% 而互动总量不升 → 说明该账号的互动主要在赞，改测收藏动机；"
                   "若你账号的数据显示赞的相关性更强 → 推翻本条，以本账号数据为准")
    if cs >= 0.12:
        return _rx("RX-06", "M7", "低", "ok",
                   "评论占互动总量 %.1f%%（高于 12%%）——讨论场健康，属稀缺状态（评论是各互动指标中与阅读量相关性最强的一项，保住它）" % (cs * 100),
                   ["把高赞评论发展成下一篇选题（读者已经把需求写在评论区）",
                    "挑 3-5 位高频评论者做「共创选题池」，形成 UGC 共创"],
                   "若评论量突然下滑而赞量不变 → 检查是否关了评论区或内容变敏感")
    return _rx("RX-06", "M7", "中", "alert",
               "评论占互动总量 %.1f%%——介于 6%%-12%% 之间，有讨论但未成场。"
               "参考：创作者大样本实证中评论数是各互动指标里与阅读量相关性最强的一项（用你自己的数据复核）" % (cs * 100),
...
def rx10_length(s):
    ...
    if not L["median"] or not L["short"] or not L["long"]:
        return _rx("RX-10", "M4", "低", "unknown", "可对照的篇幅样本不足（有摘要的近期内容 %d 条）" % L["n"],
                   ["补齐摘要字段或按正文长度重新采集后再判定"],
                   "字段补齐后若长/短差异 ≥25% → 按胜出篇幅写 SOP")
    d = L["diff_pct"]
    if d is not None and d >= 0.25:
        return _rx("RX-10", "M4", "中", "alert",
...
def rx13_allocation(s):
    ...
    if not top or not best:
        return _rx("RX-13", "M1", "中", "unknown", "题材样本不足，产能错配无法判定（需每题材 ≥%d 篇）" % sg.MIN_GROUP_N,
...
def prescribe(sig):
    ...
    for rid, fn in RULES:
        ...
        out.append(item)
    out.sort(key=lambda r: (STATE_ORDER.get(r["state"], 9),
                            SEVERITY_ORDER.get(r["severity"], 9), r["id"]))
    return out


def group_by_state(rxs):
    """处方列表 → {状态: [处方…]}（呈现层分组与「换账号是否各自不同」的断言共用同一口径）"""
    g = {k: [] for k in STATE_ORDER}
    for r in rxs:
        g.setdefault(r["state"], []).append(r)
    return g


def hit_ids(rxs):
    """本期行动清单命中的规则 id（升序）——账号之间的差异最终就落在这一组 id 上"""
    return sorted(r["id"] for r in rxs if r["state"] == "alert")


def summarize(sig, rxs):
    g = group_by_state(rxs)
    alerts = g["alert"]
    hi_all = [r for r in rxs if r["severity"] == "高"]
    headline = ("命中 %d 条行动项（其中高危 %d）· 观察 %d · 待补样本 %d · 已排除 %d"
                % (len(alerts), len([r for r in alerts if r["severity"] == "高"]),
                   len(g["watch"]), len(g["unknown"]), len(g["ok"])))
    if alerts:
        headline += "；先动这三条：%s" % " / ".join(r["id"] for r in alerts[:3])
    else:
        headline += "；本期**无命中项**——可比维度都落在正常区间，不等于账号没问题"
    if g["error"]:
        headline += "；⚠ 引擎异常 %d 条" % len(g["error"])
    return {
        ...
        "alerts": len(alerts),
        "watch": len(g["watch"]),
        "unknown": len(g["unknown"]),
        "cleared": len(g["ok"]),
        "errors": len(g["error"]),
        "alert_ids": [r["id"] for r in alerts],
        "headline": headline,
        "top_actions": [r["actions"][0] for r in alerts[:3]],
    }


def _rx_detail(r, level="###"):
    """单条处方的完整卡片（异常信号 → 对应模块 → 具体动作 → 依据 → 失效条件）"""
    out = ["",
           "%s %s · %s　[%s %s] %s" % (level, r["id"], r.get("title", ""),
                                       r["module"], r["module_name"], r["severity"]),
           "- **异常信号**：%s" % r["signal"],
           "- **对应模块**：%s %s" % (r["module"], r["module_name"]),
           "- **具体动作**："]
    out += ["  %d. %s" % (i, a) for i, a in enumerate(r["actions"], 1)]
    out += ["- **依据**：`%s`（%s）" % (r["basis"], r["basis_note"]),
            "- **失效条件**：%s" % r["falsify"]]
    return out
...
def render_md(sig, rxs):
    ...
    L = ["# 看山处方单（用户：%s · 生成 %s）" % (s["user"], datetime.date.today()),
         ...
         "",
         "> **本账号本期结论**：%s" % s["headline"],
         "",
         "## 总览（%d 个规则族：行动 %d / 观察 %d / 待补样本 %d / 已排除 %d%s）"
         % (s["total"], s["alerts"], s["watch"], s["unknown"], s["cleared"],
            "；引擎异常 %d 条" % s["errors"] if s["errors"] else ""),
         "| 编号 | 诊断项 | 模块 | 状态 | 优先 | 异常信号（摘） |",
         "|---|---|---|---|---|---|"]
    for r in rxs:
        ...
        L.append("| %s | %s | %s %s | %s | %s | %s |"
                 % (r["id"], r.get("title", ""), r["module"], r["module_name"],
                    STATE_LABELS.get(r["state"], r["state"]), r["severity"], sig_txt))
    L += ["",
          "## 一、本期行动清单（命中 %d 条）" % s["alerts"]]
    if not g["alert"]:
        L += ["", "本期无命中项——可比维度都落在正常区间。**这不是「账号没问题」，而是当前数据看不到异常**；"
                  "可对照下方「待补样本」先把证据补齐。"]
    for r in g["alert"]:
        L += _rx_detail(r)
    L += ["", "## 二、观察项（弱信号，够不上异常但值得盯）"]
    if not g["watch"]:
        L += ["", "无观察项。"]
    for r in g["watch"]:
        L += _rx_detail(r)
    ...
    if not g["unknown"]:
        L += ["", "无待补样本项。"]
    for r in g["unknown"]:
        L += ["", "- **%s · %s**：%s" % (r["id"], r.get("title", ""), r["signal"]),
              "  → 先做：%s" % r["actions"][0]]
    L += ["", "## 四、已排除（该维度不是当前瓶颈，一句话带过）"]
    if not g["ok"]:
        L += ["", "无。"]
    for r in g["ok"]:
        L += ["", "- **%s · %s**：%s" % (r["id"], r.get("title", ""), r["signal"]),
              "  → 结论：%s" % r["actions"][0]]
    if s["errors"]:
        L += ["", "## ⚠ 五、引擎异常（%d 条，属缺陷信号，不是数据结论）" % s["errors"]]
        for r in g["error"]:
            L += ["", "- **%s**：%s" % (r["id"], r["signal"])]
    L += ["", "## 口径与全局失效条件",
          ...
          "- 分组口径：15 个规则族**必然全跑一遍**（规则族是覆盖清单，避免漏检），但只有命中的才进「行动清单」；"
          "未命中按 观察／待补样本／已排除 分列——**换一个账号，行动清单的条数与内容会随之变化**"
          "（同一份规则族清单 ≠ 同一套建议）"]
    L += ["", "## 数据说明"]
